...
export default function Hardware() {
  return (
    <section id="hardware" className="w-full bg-[#0a0e16] py-16">
      <div className="max-w-[1440px] mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-3">
          <div>
            ...
            <h2 className="font-display font-bold text-[28px] md:text-[40px]">ABB TERRA AC WALLBOX 7KW SPECIFICATIONS</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
          <Reveal className="md:col-span-7">
            <div className="p-7 rounded-xl bg-[#181c24] h-full">
              <div className="flex justify-between font-mono text-[11px] text-[#76ffbb] font-bold mb-3"><span>POWER_OUTPUT // MOD_01</span><span className="px-2 py-0.5 rounded bg-[#76ffbb]/10">3X SPEED VS OUTLET</span></div>
              <div className="font-display font-bold text-[24px] md:text-[32px]">7.4 kW Continuous Single-Phase</div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-5">
                {[['Max Amperage', '32 A'], ['Supply Voltage', '230 V'], ['Frequency', '50 Hz']].map(([k, v]) => (
                  <div key={k} className="p-3 rounded bg-white/5"><div className="font-mono text-[11px] text-[#bacbbe]">{k}</div><div className="font-mono font-bold text-[22px]">{v}</div></div>
                ))}
              </div>
            </div>
          </Reveal>
          <Reveal delay={0.08} className="md:col-span-5">
            <div className="p-7 rounded-xl bg-[#181c24] h-full flex flex-col justify-between">
              <div>
              ...
              <div className="pt-5 flex gap-2 font-mono text-[11px]"><span className="px-2.5 py-1 rounded bg-white/5">Hyundai Ioniq</span><span className="px-2.5 py-1 rounded bg-white/5">Wuling EV</span><span className="px-2.5 py-1 rounded bg-white/5">BYD & Tesla</span></div>
            </div>
          </Reveal>
          {[
            ...
            <Reveal key={c.tag} delay={i * 0.06} className="md:col-span-4">
              <div className="p-7 rounded-xl bg-[#181c24] h-full flex flex-col justify-between">
                ...
                <div className="pt-4 flex justify-between font-mono text-[11px] text-[#bacbbe]"><span>{c.foot1}</span><span className="text-[#76ffbb] font-bold">{c.foot2}</span></div>
              </div>
            </Reveal>
          ))}
        </div>
